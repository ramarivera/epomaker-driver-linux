...
| Magnetic keyboards | HE60 magnetic-field reads, per-key actuation/rapid-trigger writes with complete-field readback, firmware-dependent scaling, normal/DKS/MT/toggle mode definitions with action readback, raw mode preservation and four submodes per profile | Calibration, snap pairing, recovery and hardware comparison |
