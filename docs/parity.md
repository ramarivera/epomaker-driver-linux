...
| Profiles | Model-specific profile counts and active selection; HE60 has four normal submodes per profile; model-checked configuration restore for 28 migrated keyboards and five mice; eight-profile support for five mice | Vendor export semantics and application-local magnetic caches, other models |
...
| Backups | Nine modern models' key/Fn maps, referenced macros, lighting, custom RGB pictures, sleep, debounce, OS options; RT100/Dynatab schema 6 recovery with physical Fn layer 0; seventeen magnetic models have schema 7 complete raw configuration and all 256 macros, with atomic pre-restore copies | Unreferenced macros on nonmagnetic models, screen pixels, and YC3121 manual OS options/OS-specific Fn banks |
