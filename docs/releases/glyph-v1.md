...
| G07 | Custom per-key lighting | Five picture banks; 126 writable RGB slots, readback, visual painting, draft undo/redo and JSON import/export | Correct key/LED correspondence, full visual editing/import/export workflows, bank selection and physical color verification |
| G08 | Host-driven lighting | USB live-frame transport, browser screen capture with centered ratio selection/preview and session cleanup; explicit PipeWire output-monitor discovery/capture; independent 32-band preview DSP with seven vendor-bounded controls and five-second polling cleanup | Five rhythm modes, scale/colors and optional streaming are integrated; resolve the absent third frequency group, compare the implemented position/size/rotation layout with the vendor runtime and verify physical LED correspondence and real capture/streaming; audio DSP remains independent and makes no vendor-parity claim |
...
| G10 | Clock, display language and host statistics | Clock/language commands, foreground statistics and app-controlled server refresh with disk/interface selection | Complete vendor settings/sensor comparison; verify physical output and suspend/resume behavior |
...
| G13 | Firmware management | Component-version reads, explicit metadata lookup, and local container inspection/comparison in Settings and CLI | Audit all Glyph firmware components and vendor update paths; validate images/identity, implement update/progress/reconnect and supported recovery; verify on appropriate hardware |
